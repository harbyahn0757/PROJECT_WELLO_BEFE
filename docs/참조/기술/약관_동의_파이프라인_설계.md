# 약관 동의 파이프라인 설계 및 구현

**생성일**: 미상  
**작업일자**: 미상  
**작업내용**: 약관 동의 파이프라인 설계 및 구현 (로컬·서버 동기화)

---

## 📋 개요

약관 동의 정보를 **로컬(LocalStorage)**과 **서버(PostgreSQL)**에서 각각 관리하며, 유효기간과 동기화 상태를 추적하는 완전한 파이프라인입니다.

---

## 🎯 핵심 정책

### 1. **로컬 동의 (서버 미동기화)**
- **유효기간**: 3일
- **3일 내 재방문 시**: 
  - 토스트 메시지: "YYYY년 MM월 DD일에 동의하신 약관으로 진행합니다."
  - 약관 모달 스킵, 바로 다음 단계 진행
- **3일 초과 시**: 
  - 로컬 데이터 삭제
  - 약관 동의 다시 받기

### 2. **서버 동의 (동기화 완료)**
- 로컬 체크 불필요
- 서버 데이터 신뢰
- 유효기간 제한 없음
- 타임스탬프는 서버 기준

### 3. **각 약관별 개별 관리**
- 서비스 이용약관 (필수)
- 개인정보 처리방침 (필수)
- 민감정보 수집 (필수)
- 마케팅 활용 (선택)
- **각각 독립적인 타임스탬프와 동기화 상태 보유**

---

## 📊 데이터 구조

### **1. LocalStorage 구조**

```typescript
interface IndividualTermAgreement {
  agreed: boolean;
  agreed_at: string | null;          // ISO 8601 timestamp
  expires_at: string | null;         // agreed_at + 3일
  synced_to_server: boolean;         // 서버 동기화 여부
  server_synced_at: string | null;   // 서버 동기화 시점
}

interface TermsAgreementLocal {
  uuid: string;
  partner_id: string;
  
  // 각 약관별 개별 관리
  terms_service: IndividualTermAgreement;
  terms_privacy: IndividualTermAgreement;
  terms_sensitive: IndividualTermAgreement;
  terms_marketing: IndividualTermAgreement;
  
  // 메타데이터
  last_updated: string;
  all_required_agreed: boolean;
}
```

**저장 키**: `TERMS_AGREEMENT_{uuid}_{partner_id}`

**예시**:
```json
{
  "uuid": "abc-123-def",
  "partner_id": "kindhabit",
  "terms_service": {
    "agreed": true,
    "agreed_at": "2026-01-25T10:30:00.000Z",
    "expires_at": "2026-01-28T10:30:00.000Z",
    "synced_to_server": true,
    "server_synced_at": "2026-01-25T10:30:05.000Z"
  },
  "terms_privacy": {
    "agreed": true,
    "agreed_at": "2026-01-25T10:30:00.000Z",
    "expires_at": "2026-01-28T10:30:00.000Z",
    "synced_to_server": true,
    "server_synced_at": "2026-01-25T10:30:05.000Z"
  },
  "terms_sensitive": {
    "agreed": true,
    "agreed_at": "2026-01-25T10:30:00.000Z",
    "expires_at": "2026-01-28T10:30:00.000Z",
    "synced_to_server": false,
    "server_synced_at": null
  },
  "terms_marketing": {
    "agreed": false,
    "agreed_at": null,
    "expires_at": null,
    "synced_to_server": false,
    "server_synced_at": null
  },
  "last_updated": "2026-01-25T10:30:00.000Z",
  "all_required_agreed": true
}
```

---

### **2. PostgreSQL 구조**

**테이블**: `welno.welno_patients`

**추가 컬럼**:
```sql
-- JSONB 형식으로 각 약관별 상세 정보 저장
terms_agreement_detail JSONB

-- 모든 필수 약관 동의 완료 시점
terms_all_required_agreed_at TIMESTAMPTZ
```

**JSONB 구조**:
```json
{
  "terms_service": {
    "agreed": true,
    "agreed_at": "2026-01-25T10:30:00Z",
    "version": "v1.0"
  },
  "terms_privacy": {
    "agreed": true,
    "agreed_at": "2026-01-25T10:30:00Z",
    "version": "v1.0"
  },
  "terms_sensitive": {
    "agreed": true,
    "agreed_at": "2026-01-25T10:30:00Z",
    "version": "v1.0"
  },
  "terms_marketing": {
    "agreed": false,
    "agreed_at": null,
    "version": "v1.0"
  }
}
```

---

## 🔄 전체 플로우

### **시나리오 1: 최초 방문 (로컬/서버 데이터 없음)**

```
1. 사용자 캠페인 진입
   ↓
2. checkAllTermsAgreement() 호출
   ↓
3. 서버 체크 → 없음
   ↓
4. 로컬 체크 → 없음
   ↓
5. needsAgreement: true
   ↓
6. 약관 동의 모달 표시
   ↓
7. 사용자 "동의하고 시작하기" 클릭
   ↓
8. saveTermsAgreement() 호출
   ↓
9. LocalStorage 저장 (synced_to_server: false, expires_at: +3일)
   ↓
10. 서버 API 호출 (/api/v1/campaigns/disease-prediction/register-patient/)
   ↓
11. DB 저장 (terms_agreement_detail JSONB)
   ↓
12. LocalStorage 업데이트 (synced_to_server: true)
   ↓
13. 다음 단계 진행
```

---

### **시나리오 2: 재방문 (로컬 동의 있음, 3일 이내, 서버 미동기화)**

```
1. 사용자 캠페인 진입
   ↓
2. checkAllTermsAgreement() 호출
   ↓
3. 서버 체크 → 없음
   ↓
4. 로컬 체크 → 있음
   ↓
5. 유효기간 체크 → 3일 이내 (유효)
   ↓
6. synced_to_server 체크 → false
   ↓
7. needsAgreement: false, showReminderToast: true
   ↓
8. 토스트 표시: "2026년 1월 25일에 동의하신 약관으로 진행합니다."
   ↓
9. 약관 모달 스킵
   ↓
10. 다음 단계 진행
```

---

### **시나리오 3: 재방문 (서버 동의 있음)**

```
1. 사용자 캠페인 진입
   ↓
2. checkAllTermsAgreement() 호출
   ↓
3. 서버 체크 → 있음 (terms_agreement_detail)
   ↓
4. 필수 약관 모두 동의 확인
   ↓
5. needsAgreement: false, showReminderToast: false
   ↓
6. 토스트 표시 없음 (서버 데이터 신뢰)
   ↓
7. 약관 모달 스킵
   ↓
8. 다음 단계 진행
```

---

### **시나리오 4: 재방문 (로컬 동의 만료, 3일 초과)**

```
1. 사용자 캠페인 진입
   ↓
2. checkAllTermsAgreement() 호출
   ↓
3. 서버 체크 → 없음
   ↓
4. 로컬 체크 → 있음
   ↓
5. 유효기간 체크 → 3일 초과 (만료)
   ↓
6. isExpired: true
   ↓
7. LocalStorage 데이터 삭제
   ↓
8. needsAgreement: true
   ↓
9. 약관 동의 모달 표시 (처음부터 다시)
```

---

## 🛠️ 구현된 파일

### **프론트엔드**

1. **`src/utils/termsAgreement.ts`** (NEW)
   - `checkAllTermsAgreement()`: 약관 동의 체크 (로컬 + 서버)
   - `saveTermsAgreement()`: 약관 동의 저장 (로컬 + 서버)
   - `clearLocalTermsAgreement()`: 로컬 데이터 삭제

2. **`src/components/AuthForm.tsx`** (MODIFIED)
   - `onConfirm` 핸들러 개선
   - `saveTermsAgreement()` 유틸 사용
   - 각 약관별 타임스탬프 전달

3. **`src/campaigns/disease-prediction/index.tsx`** (MODIFIED)
   - 진입 시 `checkAllTermsAgreement()` 호출
   - 토스트 메시지 표시
   - 약관 필요 시 자동 리다이렉트

### **백엔드**

1. **`app/api/v1/endpoints/terms_agreement.py`** (NEW)
   - `GET /api/v1/terms/check`: 약관 동의 확인
   - `GET /api/v1/terms/status/{uuid}`: 약관 상태 상세 조회

2. **`app/services/wello_data_service.py`** (MODIFIED)
   - `save_terms_agreement_detail()`: 약관 상세 정보 저장

3. **`app/api/v1/endpoints/campaign_payment.py`** (MODIFIED)
   - `register-patient` API: `terms_agreement_detail` 처리 추가

4. **`app/api/v1/__init__.py`** (MODIFIED)
   - `terms_agreement` 라우터 등록

### **데이터베이스**

```sql
ALTER TABLE welno.welno_patients 
ADD COLUMN IF NOT EXISTS terms_agreement_detail JSONB,
ADD COLUMN IF NOT EXISTS terms_all_required_agreed_at TIMESTAMPTZ;
```

---

## 🧪 테스트 방법

### **1. 최초 방문 테스트**
1. 브라우저 시크릿 모드 열기
2. `http://localhost:9282/campaigns/disease-prediction?uuid=test001&partner=kindhabit` 접속
3. 약관 동의 모달 표시 확인
4. 동의 → LocalStorage 저장 확인 (개발자 도구)
5. 서버 저장 확인 (DB 쿼리)

### **2. 재방문 테스트 (3일 이내)**
1. 같은 UUID로 재접속
2. 토스트 메시지 표시 확인
3. 약관 모달 스킵 확인

### **3. 만료 테스트**
1. LocalStorage 데이터 수동 수정 (expires_at을 과거로)
2. 재접속
3. 약관 모달 다시 표시 확인

### **4. 서버 동기화 테스트**
```bash
# DB 확인
cd /home/workspace/PROJECT_WELNO_BEFE/planning-platform/backend
python3 << 'EOF'
import sys, os
sys.path.insert(0, os.getcwd())
from app.core.database import db_manager
import asyncio

async def check():
    query = """
    SELECT uuid, name, terms_agreement_detail, terms_all_required_agreed_at
    FROM welno.welno_patients
    WHERE uuid = 'test001'
    """
    result = await db_manager.execute_one(query)
    if result:
        print("✅ 서버 동의 확인:")
        print(f"  UUID: {result['uuid']}")
        print(f"  이름: {result['name']}")
        print(f"  약관 상세: {result['terms_agreement_detail']}")
        print(f"  동의 시간: {result['terms_all_required_agreed_at']}")

asyncio.run(check())
EOF
```

---

## 🔍 API 엔드포인트

### **GET /api/v1/terms/check**

**요청**:
```
GET /api/v1/terms/check?uuid=abc-123&partner_id=kindhabit
```

**응답**:
```json
{
  "agreed": true,
  "terms_detail": {
    "terms_service": {
      "agreed": true,
      "agreed_at": "2026-01-25T10:30:00Z"
    },
    "terms_privacy": {
      "agreed": true,
      "agreed_at": "2026-01-25T10:30:00Z"
    },
    "terms_sensitive": {
      "agreed": true,
      "agreed_at": "2026-01-25T10:30:00Z"
    },
    "terms_marketing": {
      "agreed": false,
      "agreed_at": null
    }
  },
  "terms_agreed_at": "2026-01-25T10:30:00Z",
  "message": "약관 동의 정보 조회 완료"
}
```

### **POST /api/v1/campaigns/disease-prediction/register-patient/**

**요청**:
```json
{
  "uuid": "abc-123",
  "oid": "ORDER_123",
  "user_info": {
    "name": "홍길동",
    "phone": "01012345678",
    "birth": "1990-01-01",
    "gender": "M"
  },
  "terms_agreement_detail": {
    "terms_service": {
      "agreed": true,
      "agreed_at": "2026-01-25T10:30:00Z"
    },
    "terms_privacy": {
      "agreed": true,
      "agreed_at": "2026-01-25T10:30:00Z"
    },
    "terms_sensitive": {
      "agreed": true,
      "agreed_at": "2026-01-25T10:30:00Z"
    },
    "terms_marketing": {
      "agreed": false,
      "agreed_at": null
    }
  },
  "api_key": "partner_api_key",
  "partner_id": "kindhabit"
}
```

---

## 📈 타임스탬프 파이프라인

```
[사용자 동의 버튼 클릭]
    ↓
[프론트엔드: 타임스탬프 생성]
  now = new Date()
  expires_at = now + 3일
    ↓
[LocalStorage 저장]
  {
    terms_service: {
      agreed: true,
      agreed_at: "2026-01-25T10:30:00.000Z",
      expires_at: "2026-01-28T10:30:00.000Z",
      synced_to_server: false,
      server_synced_at: null
    },
    ...
  }
    ↓
[서버 API 호출]
  POST /api/v1/campaigns/disease-prediction/register-patient/
    ↓
[백엔드: DB 저장]
  UPDATE welno.welno_patients 
  SET terms_agreement_detail = {...},
      terms_all_required_agreed_at = NOW()
    ↓
[프론트엔드: LocalStorage 업데이트]
  {
    terms_service: {
      ...
      synced_to_server: true,
      server_synced_at: "2026-01-25T10:30:05.000Z"
    },
    ...
  }
```

---

## ⚙️ 주요 함수

### **프론트엔드**

#### `checkAllTermsAgreement(uuid, partnerId)`
- 서버 → 로컬 순서로 체크
- 각 약관별 유효성 검증
- 만료된 약관 자동 삭제
- 토스트 메시지 생성

#### `saveTermsAgreement(uuid, partnerId, termsAgreement, ...)`
- 로컬 저장 (각 약관별 타임스탬프)
- 서버 저장 (비동기)
- 서버 성공 시 로컬 동기화 플래그 업데이트

### **백엔드**

#### `GET /api/v1/terms/check`
- 환자 UUID로 약관 동의 조회
- 각 약관별 상세 정보 반환

#### `save_terms_agreement_detail(uuid, hospital_id, terms_agreement_detail)`
- JSONB로 약관 상세 저장
- 필수 약관 모두 동의 시 `terms_all_required_agreed_at` 업데이트

---

## 🎯 향후 개선 사항

1. **약관 버전 관리**
   - 약관 내용 변경 시 버전 추적
   - 구버전 동의자에게 재동의 요청

2. **동기화 재시도 로직**
   - 서버 저장 실패 시 백그라운드 재시도
   - Service Worker 활용

3. **분석 및 통계**
   - 약관별 동의율 대시보드
   - 만료 후 재동의율 분석

4. **마케팅 약관 별도 UI**
   - 필수 약관과 선택 약관 시각적 분리
   - 마케팅 약관 나중에 동의 옵션

---

## 📝 작성일

2026-01-25

## 작성자

AI Assistant (Claude Sonnet 4.5)
