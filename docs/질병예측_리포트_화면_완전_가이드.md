...
graph TD
    ...
    D --> D1[HealthAgeSection<br/>건강나이]
    D --> D2[RankSection<br/>등수]
    D --> D3[ProbabilitySection<br/>발병확률]
    D --> D4[DiseasesSection<br/>질병/암 리스트]
    ...
    D2 --> D2A[내 건강나이 등수<br/>메인]
    D2 --> D2B[좋은 항목 2개<br/>🥇🥈]
    D2 --> D2C[구분선]
    D2 --> D2D[나쁜 항목 2개<br/>⚠️]
    
    D3 --> D3A[요약 메시지]
    D3 --> D3B[원형 차트 2x2<br/>상위 4개]
    
    D4 --> D4A[CancerSection<br/>암]
    D4 --> D4B[DiseaseSection<br/>질병]
...
sequenceDiagram
    ...
    participant F as 프론트엔드<br/>(DiseaseReportPage)
    participant H as Hooks<br/>(useUnifiedStatus)
    participant B as 백엔드<br/>(API)
    participant WS as WebSocket
    participant DB as 데이터베이스
    
    U->>F: 페이지 접근<br/>/disease-report?uuid=...&hospital_id=...
    F->>F: URL 파라미터 파싱<br/>(uuid, hospital_id, oid, sessionId)
    
    F->>H: useUnifiedStatus 호출
    H->>B: 통합 상태 조회<br/>GET /api/v1/disease-report/check-partner-status
    B->>DB: 약관/결제/데이터/리포트 확인
    DB-->>B: 상태 정보 반환
    B-->>H: 통합 상태 반환
    H-->>F: status 반환
    
    alt 자동 리다이렉트 필요
        F->>F: useMatrixAutoRedirect<br/>(약관/결제/데이터 수집 페이지로 이동)
        F-->>U: 리다이렉트
    else 리포트 조회 가능
        F->>B: 리포트 조회<br/>GET /api/v1/welno/mediarc-report?uuid=...
        B->>DB: 리포트 데이터 조회<br/>(welno_mediarc_reports)
        DB-->>B: mediarc_response 반환
        B->>B: URL 만료 확인<br/>(HEAD 요청)
        B-->>F: 리포트 데이터 반환<br/>(bodyage, rank, disease_data, cancer_data)
        
        F->>F: 데이터 변환<br/>(disease + cancer 통합)
        F->>F: 정렬 로직<br/>(ABNORMAL > BOUNDARY > NORMAL)
        F->>F: 나이 계산<br/>(생년월일 기반)
        F->>F: setReportData(aimsData)
        F->>F: 애니메이션 트리거<br/>(패널 표시, 반짝임 효과)
        F-->>U: 리포트 화면 렌더링
    end
    
    opt 리포트 생성 대기 (sessionId 존재)
        F->>WS: WebSocket 연결<br/>ws://.../tilko/ws/{sessionId}
        WS-->>F: 연결 완료
    ...
    U->>F: 다운로드 버튼 클릭
    F->>B: PDF 다운로드 요청<br/>GET /api/v1/welno/mediarc-report/download
    B->>DB: report_url 조회
    DB-->>B: S3 URL 반환
    B->>B: S3 파일 다운로드
    B-->>F: PDF 파일 반환
