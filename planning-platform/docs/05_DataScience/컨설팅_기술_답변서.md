...
**현재 상태: 가능 (통합 구조 구축 완료)**

파트너사가 발급하는 `web_app_key`(환자별 고정값)가 모든 데이터 소스에 공통 키로 저장되어, SQL JOIN만으로 즉시 통합 분석이 가능하다.

| 데이터 소스 | 저장 위치 | 컬럼명 | 실제 값 |
|------------|----------|--------|--------|
| 검진 데이터 | partnerData (JSON) | 위젯 파라미터 `web_app_key` | `web_app_key` |
| 상담 (RAG Chat) | `tb_partner_rag_chat_log` (PostgreSQL) | `user_uuid` | `web_app_key` |
| 만족도 조사 | `tb_hospital_survey_responses` (PostgreSQL) | `respondent_uuid` | `web_app_key` |
| 행동 이벤트 | `medilinx-logs-data` (Elasticsearch) | `data.user.webAppKey` | `web_app_key` |

**공통 식별자: `web_app_key`**
- 파트너사(Jerry/Kraken)가 환자별 고정값으로 발급 (32자 hex hash)
- 채팅 위젯 → `user_uuid`로 저장, 설문 위젯 → `respondent_uuid`로 저장, ES → `webAppKey`로 저장
- 컬럼명은 다르지만 **값은 동일한 `web_app_key`**

### 상세 질문 답변

**"특정 수치가 나빠서 상담을 길게 한 사람이 병원 만족도가 낮은지" 추적 가능한가?**

> **가능.** `web_app_key`로 3개 테이블을 JOIN하면 아래 분석이 즉시 가능:
> ```sql
> SELECT c.user_uuid, t.risk_level, t.engagement_score, s.overall_satisfaction
> FROM welno.tb_partner_rag_chat_log c
> JOIN welno.tb_chat_session_tags t ON c.session_id = t.session_id
> JOIN welno.tb_hospital_survey_responses s ON c.user_uuid = s.respondent_uuid
> ```
> - 검진 이상 항목 수 vs 상담 시간(메시지 수) 상관관계
> - 상담 깊이(conversation_depth) vs 만족도 점수 교차분석
> - 위험 태그 보유 환자의 만족도 평균 비교
>
> *(현재 DB 기준: 채팅 29명, 설문 75명이 아직 겹치지 않으나, 데이터가 축적되면 자동으로 교차 분석 가능)*
...
```
[ES 이벤트]     →  [채팅 세션]     →  [설문 응답]
webAppKey             user_uuid          respondent_uuid
   ↓                     ↓                    ↓
   └─────── 모두 동일한 web_app_key (환자별 고정값) ───────┘
                         ↓
...
> **구조적으로 가능.** 설문 응답(`respondent_uuid`)과 채팅(`user_uuid`)이 동일한 `web_app_key`로 저장되어 있으므로, JOIN 기반 교차분석 쿼리로 구현 가능:
> - 검진 항목별 만족도 교차분석
> - 이상 소견 유무별 재방문 의향 비교
> - 상담 깊이별 만족도 상관관계
>
> 현재 백오피스 UI에는 이 교차분석 뷰가 아직 없으며, 추가 개발로 제공 예정.
...
| 영역 | 현재 수준 | 단기 확장 가능성 |
|------|----------|----------------|
| 데이터 통합 (UUID 매핑) | `web_app_key` 기반 통합 완료 | **높음** — SQL JOIN으로 즉시 교차분석 가능 |
| 태깅/라벨링 | LLM + Rule-based 하이브리드 완성 | **높음** — 프롬프트/사전 수정으로 커스텀 |
| 전환 최적화 | 퍼널 추적만 구현 | **중간** — 매칭 룰 엔진 추가 필요 |
| 병원 CRM 분석 | 평균/추이/분포 제공 | **높음** — `web_app_key` JOIN으로 교차분석 가능 |
