...
class MdxWelnoSyncService:
    """mdx_agr_list ↔ welno 동기화 서비스"""
    ...
    async def get_mdx_patients_by_combo(
        ...
        """
        welno → mdx: 전화번호, 생년월일, 이름으로 mdx 환자 조회
    ...
    async def get_wello_patient_by_combo(
        ...
        """
        mdx → welno: 전화번호, 생년월일, 이름으로 welno 환자 조회
        단일 환자 레코드 반환
        """
        try:
            conn = await asyncpg.connect(**self.db_config)
