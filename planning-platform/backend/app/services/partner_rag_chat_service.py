...
class PartnerRagChatService(WelnoRagChatService):
    ...
    # P2-4: HOOK_ANGLES 다양화 — 8 → 24 (5/3+5/4 사용자 평가 "교과서적/단조" 대응)
    # 사용자 다회 노출 시 단조로움 차단. 전체 톤 (담백 / 공감 / 호기심) 균형 유지.
    HOOK_ANGLES = [
        # — 담백 안내 (정보 전달)
        ...
        "검진 결과 중 흥미로운 패턴이 보였다는 호기심 유도 (단정 X). 같이 들여다보자는 권유.",
        "결과 정리하면서 짚어드릴 만한 게 보였다는 짧은 큐레이션 멘트. 부담 X.",
        "검진 데이터를 함께 읽어드릴 시간이 됐다는 차분한 안내. 도구가 아닌 동행자 톤.",
        # — 항목 구체 언급 (수치/부위)
        "구체적 검진 항목(혈압/혈당/콜레스테롤/간수치 등) 중 하나를 짧게 언급하면서 같이 보자고 권유. 진단 강요 X.",
        "특정 수치 한 가지가 살짝 신경 쓰이는 범위라고 짧게 언급 (단정 X). 같이 보자는 권유.",
        "한두 항목이 가이드라인 기준에서 살짝 벗어났다는 정보 전달. 안심시키는 톤.",
        "이번 검진의 키 포인트 1개를 짧게 짚으며 자세히 알려드리겠다는 약속.",
        # — 공감/걱정 살피기
        "환자가 평소 신경 썼을 만한 부분을 짚어주는 진중한 관심 표현. 부담스럽지 않게.",
        "환자의 걱정/궁금증을 먼저 묻는 공감 톤. 의문형으로 마무리(~있으세요?). {name}님 호칭 + 부드러운 권유. 친구가 안부 묻듯, 부담 없이.",
        "최근 컨디션이나 일상 변화에 대한 가벼운 안부 + 결과 보면서 같이 살피자는 권유.",
        "혹시 마음에 담아두신 검진 항목이 있는지 부드럽게 묻는 톤. 의문형, 가볍게.",
        "검진 결과 받으시기 전 미리 챙기고 싶은 부분이 있는지 묻는 따뜻한 안내.",
        # — 행동 제안 / 다음 단계
        "다음 단계 (재검/관리/생활습관) 안내가 준비됐다는 짧은 알림. 결정은 환자에게 맡김.",
        "결과를 바탕으로 일상에서 챙기면 좋을 부분 한 가지가 있다는 짧은 큐레이션.",
        "이번 결과로 함께 점검할 것 1가지가 있다는 안내. 부담 없이.",
        "수치 변화 추이가 의미 있게 보였다는 짧은 멘트 (단정 X). 같이 본격 분석하자는 초대.",
        # — 시간/계절 활용 (자연스러운 변주)
        "이번 시즌 (요즘) 챙기면 좋을 부분이 결과에 보였다는 자연 톤.",
        "계절상 변화 가능성이 있는 항목 1개를 짚으며 안내. 부담 X.",
        "최근 검진 트렌드와 결과를 빗대어 안내하는 정보 톤. 광고 어조 X.",
    ]

    # GREETING_ANGLES 4 → 12 — hook 의 흐름 이어받기
    GREETING_ANGLES = [
        # — 차분 초대
        "후킹의 자연스러운 연장 + 어디부터 볼지 묻는 차분한 초대 톤.",
        "결과 정리됐다는 사실 + 가볍지 않게 시작하자는 자연 톤.",
        "결과 화면을 함께 열어보자는 동행자 톤. 어색한 인사말 X.",
        # — 항목 안내
        "구체 항목 1~2개를 짧게 언급 + 무엇이 궁금한지 묻는 도움 톤.",
        "결과의 키 포인트를 1개만 짚어주고 추가 궁금증을 유도하는 큐레이션 톤.",
        "검진 데이터의 흐름 (전체 → 항목 → 수치) 중 어느 깊이까지 보고 싶은지 묻는 안내.",
        # — 공감
        "환자 감정/걱정을 먼저 살피며 마음 열게 하는 공감 톤. 의문형 권유.",
        "최근 일상 컨디션과 검진 결과를 연결지어 같이 살펴보자는 안내. 따뜻하게.",
        "환자가 부담 없이 질문할 수 있게 분위기 만드는 부드러운 시작 멘트.",
        # — 행동 제안
        "결과를 바탕으로 일상에서 챙기면 좋을 부분 1가지를 짧게 제안하고 더 알려드릴 준비됐다는 안내.",
        "다음 단계 (재검/생활관리) 중 어느 것부터 안내해 드릴까요? 라는 선택형 권유.",
        "검진 결과를 어떻게 활용하고 싶은지 (안심 / 관리 / 재검) 짧게 묻는 톤.",
    ]
    ...
    # P2-4: 정상인용 (hook, chat_greeting) 쌍 4 → 16 (단조 노출 차단)
    # 동일 환자 재방문 시 같은 메시지 반복 회피. 모든 쌍은 (담백 / 따뜻 / 명확) 톤 분포.
    NORMAL_GREETING_PAIRS = [
        ...
        # — 따뜻 톤
        (
            "{name}님, 검진 결과 잘 받으셨어요 — 함께 살펴볼까요? 💬",
            "{name}님, 결과 전반적으로 안정적이에요. 일상에서 챙기면 좋을 부분이 있는지 같이 짚어볼까요? 😊",
        ),
        (
            "{name}님, {hospital} 결과지 정리 완료! 편하게 물어보세요 ✨",
            "결과는 전체적으로 양호해요, {name}님. 평소 신경 쓰이는 수치나 항목 있으면 알려주세요 💡",
        ),
        # — 행동 제안
        (
            "{name}님, 검진 결과 + 함께 챙길 포인트 정리해 드릴게요 📋",
            "{name}님, 결과는 양호한 편이에요. 앞으로 어떻게 관리하면 좋을지 안내해 드릴까요? 😊",
        ),
        (
            "{name}님! {hospital} 결과 + 일상 관리 가이드 준비됐어요 ✨",
            "{name}님, 전체 양호한 결과 위에 일상에서 챙길 만한 부분 1~2개 추려봤어요. 함께 볼까요? 💬",
        ),
        # — 호기심 자극 (담백)
        (
            "{name}님, 검진 결과에서 흥미로운 패턴이 보여요 💡",
            "{name}님, 결과 흐름이 안정적인데 살짝 짚을 부분이 있어요. 어떤 항목부터 보시겠어요? 😊",
        ),
        (
            "{hospital} 결과 보면서 {name}님께 안내드릴 게 있어요 📊",
            "{name}님, 큰 문제 없는 결과예요! 다만 자주 받는 질문 위주로 정리해 드릴까요? 💬",
        ),
        # — 시즌/일상 연결
        (
            "{name}님, 요즘 컨디션 어떠세요? 결과 같이 보면서 이야기해요 ☕",
            "{name}님, 검진 결과는 양호한 편이에요. 최근 일상에서 챙기고 계신 게 있다면 같이 점검해 봐요 💡",
        ),
        (
            "{name}님, 결과지 받았으니 천천히 함께 보실까요? 😊",
            "{name}님, 결과 보면서 평소 신경 썼던 부분이 어떻게 나왔는지 같이 확인해 봐요 ✨",
        ),
        # — 안심 + 다음 단계
        (
            "{name}님, {hospital} 결과 정리 완료! 안심하셔도 좋겠어요 🌿",
            "{name}님, 결과 전반적으로 양호해요. 다음 검진까지 챙기면 좋은 게 있는지 함께 확인해 볼까요? 😊",
        ),
        (
            "{name}님, 결과 안내드릴 준비 됐어요 💬",
            "{name}님, 전체 결과는 좋아 보여요. 추가로 알고 싶은 건강 주제가 있으면 편하게 말씀해 주세요 ✨",
        ),
        # — 짧고 친근
        (
            "{name}님, 검진 결과 도착! {hospital} 함께 볼까요? 📋",
            "{name}님, 결과 양호해요. 어디부터 살펴드릴까요? 😊",
        ),
        (
            "{name}님, {hospital} 결과 안내드릴게요 ✨",
            "{name}님, 전반적으로 안정적이에요. 궁금한 부분 있으세요? 💬",
        ),
    ]
