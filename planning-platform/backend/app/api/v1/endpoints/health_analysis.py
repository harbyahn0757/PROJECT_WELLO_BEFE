...
2. **균형 잡힌 발견사항**: 실제 데이터에 따라 적절한 개수 제시 - 문제가 있는 지표만 "걱정" 상태로, 정상인 지표는 "좋음" 상태로 포함하여 균형 유지
...
4. **데이터 기반**: 실제 검진 날짜, 수치를 명시하여 구체적 근거 제시 - 좋아지거나 나빠지는 추세라면 이를 활용해서 "개선되고 있으니 조금 더 노력해볼까요", "관리가 필요하지만 충분히 개선 가능합니다" 등 격려 메시지 포함
5. **동기부여 강화**: 
   - 개선 추세: "훌륭한 개선입니다! 이 상태를 유지해보세요", "좋은 방향으로 가고 있어요"
   - 정상 범위: "현재 건강한 상태를 잘 유지하고 계시네요", "이대로 계속 관리해주세요"
   - 주의 필요: "관리가 필요하지만 충분히 개선 가능합니다", "작은 변화로도 큰 효과를 볼 수 있어요"
6. **학술 근거**: 각 판단에 대한 의학적 근거와 출처 명시 , 년도 논문 출처 등 
7. **위험 요소 통합**: 중복되는 위험 요소는 하나로 통합하여 제시
8. **추세 분석**: 수치 변화의 의미와 향후 전망을 포함
9. **처방약 연계 분석**: 
   - 혈압약 복용 중 + 혈압 개선: "혈압약 효과가 잘 나타나고 있어요! 꾸준한 복용과 생활습관 개선으로 더 좋아질 수 있습니다"
   - 당뇨약 복용 중 + 혈당 관리: "약물 치료와 함께 식단 관리를 병행하시면 더 좋은 결과를 기대할 수 있어요"
   - 검진 시점과 처방 시점을 연계하여 약물 효과 분석 및 격려
10. **주요 지표 분석**: 제공된 검진 데이터의 주요 지표들(체중, 혈압, 혈당, 콜레스테롤 등)을 분석하되, 정상인 지표도 긍정적 발견사항으로 포함하여 동기부여 
11. **발견사항 예시**:
    - 건강한 사람: "심혈관 건강 우수", "간 기능 정상" 등 긍정적 발견사항 위주
    - 문제 있는 사람: "고도비만 주의", "당뇨전단계 관리" 등 주의사항과 함께 좋은 점도 포함
12. JSON 형식을 정확히 준수
