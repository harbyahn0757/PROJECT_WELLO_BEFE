...
class ChatSessionManager:
    ...
    def _get_history_key(self, uuid: str, hospital_id: str) -> str:
        """채팅 히스토리 키 생성"""
        return f"welno:chat:history:{uuid}:{hospital_id}"
    
    def add_message(self, uuid: str, hospital_id: str, role: str, content: str):
        """메시지 추가 (JSON 리스트 형태)"""
        if not self.redis_client:
            return
        
        key = self._get_history_key(uuid, hospital_id)
        message = {
            "role": role,
            "content": content,
    ...
    def get_history(self, uuid: str, hospital_id: str) -> List[Dict[str, Any]]:
        """전체 히스토리 조회"""
        if not self.redis_client:
            return []
    ...
    def clear_history(self, uuid: str, hospital_id: str):
        """히스토리 초기화"""
        if not self.redis_client:
            return
        key = self._get_history_key(uuid, hospital_id)
