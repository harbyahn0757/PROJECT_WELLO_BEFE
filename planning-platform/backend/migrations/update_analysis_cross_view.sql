-- v_analysis_cross 뷰 갱신 — 병원 태깅 컬럼 + partner_type JOIN 추가
...
SELECT
  ...
  -- 병원 태깅 필드 (NEW)
  t.medical_tags, t.lifestyle_tags,
  t.medical_urgency, t.anxiety_level,
  t.prospect_type, t.hospital_prospect_score,
  -- 파트너 유형 (NEW)
...
LEFT JOIN welno.tb_partner_config pc ON pc.partner_id = c.partner_id AND pc.is_active = true
LEFT JOIN LATERAL (
