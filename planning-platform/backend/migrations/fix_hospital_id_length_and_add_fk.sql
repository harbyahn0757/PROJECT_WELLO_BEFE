-- hospital_id 컬럼 크기 조정 및 외래키 제약조건 추가
-- 실행일: 2026-02-09
-- 목적: 긴 해시 ID 지원 및 데이터 참조 무결성 보장

BEGIN;

-- 1. hospital_id 컬럼 크기 확장 (VARCHAR(50) → VARCHAR(255))
ALTER TABLE welno.welno_hospitals 
ALTER COLUMN hospital_id TYPE VARCHAR(255);

-- 다른 테이블의 hospital_id도 동일하게 확장
...
SELECT DISTINCT 
    c.hospital_id,
    COALESCE(c.hospital_name, '병원_' || LEFT(c.hospital_id, 20)) as hospital_name,
    c.partner_id
...
-- 3. 데이터 정합성 재검증
SELECT 'tb_hospital_rag_config 참조 무결성 재검증' as check_name,
...
-- 5. 파트너 참조 무결성 확인 (tb_partner_config가 있는 경우)
DO $$
BEGIN
    IF EXISTS (
...
-- 8. 병원 데이터 현황 확인
SELECT 'welno_hospitals 데이터 현황' as info,
       partner_id,
       COUNT(*) as hospital_count,
       MIN(LENGTH(hospital_id)) as min_id_length,
       MAX(LENGTH(hospital_id)) as max_id_length
...
-- 마이그레이션 완료 메시지
SELECT '✅ Hospital ID 크기 조정 및 외래키 제약조건 마이그레이션 완료' as status,
       'hospital_id VARCHAR(255) 확장, 참조 무결성 보장' as details;