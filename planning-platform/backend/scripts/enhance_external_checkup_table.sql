...
-- 1. 성능 지표 관련 컬럼 추가
ALTER TABLE wello.wello_external_checkup_items
...
-- 코멘트 추가
COMMENT ON COLUMN wello.wello_external_checkup_items.sensitivity IS '민감도 (%)';
COMMENT ON COLUMN wello.wello_external_checkup_items.specificity IS '특이도 (%)';
COMMENT ON COLUMN wello.wello_external_checkup_items.auc_score IS 'AUC 점수 (0-1)';
COMMENT ON COLUMN wello.wello_external_checkup_items.study_design IS '연구 설계: 전향/후향, 단일/다기관';
COMMENT ON COLUMN wello.wello_external_checkup_items.sample_size IS '주요 임상연구 샘플 수 (N)';
COMMENT ON COLUMN wello.wello_external_checkup_items.mfds_approval IS 'MFDS 허가 여부';
COMMENT ON COLUMN wello.wello_external_checkup_items.reimbursement_status IS '급여 상태: 급여/선별급여/비급여';
